$(document).on('turbolinks:load', function(){
  $('#price_calc').on('input', function(){   //リアルタイムで表示したいのでinputを使う｡入力の度にイベントが発火するようになる｡
    ...
    $('.SalesCommission').html(fee.toLocaleString()) //  手数料の表示｡html()は追加ではなく､上書き｡入力値が変わる度に表示も変わるようにする｡
    $('.SalesCommission').prepend('¥') // 手数料の前に¥マークを付けたいので
    $('.SalesCommission_2').html(profit.toLocaleString())
    $('.SalesCommission_2').prepend('¥')
    $('#price').val(profit) // 計算結果を格納用フォームに追加｡もし､入力値を追加したいのなら､今回はdataを引数に持たせる｡
    if(profit == '') {   // もし､計算結果が''なら表示も消す｡
    $('.SalesCommission_2').html('');
